...
namespace NEA
{
    public partial class Registration : Form
    {
                ...
                if (phoneValid == true && emailValid == true && isSame == true)
                {
                    ...
                    using (SqlConnection con = new SqlConnection(@"data source=ALEXS-PC;initial catalog=cafeDB;trusted_connection=true"))
                    {
                        ...
                        try
                        {
                            ...
                            using (SqlCommand cmdRegister = new SqlCommand("INSERT INTO tbl_staff VALUES" +
                                "(@staffID, @username, @password, @phonenumber, @email, @admin, @firstname, @lastname)", con))
                            {
                        ...
                        try
                        {
                            Form menu = new Main_Menu //A new main menu form is created and values are passed to it
                            {
                                Home = Home,
                                StaffID = staffID,
                                Firstname = firstname,
        ...
        public static bool ValidNum(string number)
        {
            ...
            if (regexPhone.IsMatch(number) == false)
            { return false; }
            else
            { return true; }
        }
